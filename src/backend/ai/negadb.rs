...
use super::structures::database::Database;
...
impl AI for NegaDB{
	fn new() -> Self{
		NegaDB{memo: MemoMap::new(), depth: INITIAL_DEPTH, killerray: Killerray::new(), time: INITIAL_TIME, database: Database::new()}
	}
	...
	fn search(&mut self, mut b: Board) -> Move{
        if let Some(m) = self.database.get(&mut b){
            return m;
        }
...
impl NegaDB{
	...
	fn alphabeta(&mut self, b: &mut Board, mut alpha: Score, mut beta: Score, depth: usize, time: &Instant) -> Score{
        ...
        if let Some(m) = self.database.get(b){
            println!("I found a move :)");
            b.move_piece(&m);
