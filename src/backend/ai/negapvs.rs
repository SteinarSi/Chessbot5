...
impl AI for NegaPVS{
	fn new() -> Self{
		NegaPVS{memo: MemoMap::new(), depth: INITIAL_DEPTH, killerray: Killerray::new()}
	}
...
impl NegaPVS{
	...
	fn alphabeta(&mut self, b: &mut Board, mut alpha: Score, mut beta: Score, depth: usize) -> Score{
		...
		for m in iter{
			...
			b.move_piece(&m);
			let mut value = - self.alphabeta(b, -alpha-1, -alpha, depth-1); // Null window search
			if value > alpha && value < beta {
				value = - self.alphabeta(b, -beta, -alpha, depth-1); // Open window search
				if value > alpha{
					alpha = value;
					exact = true;
					best = Some(m);
